...
public typealias OnboardingItemInfo = (imageName: String, title: String, description: String, iconName: String, color: UIColor, titleColor: UIColor, descriptionColor: UIColor, titleFont: UIFont, descriptionFont: UIFont, backgroundImage: UIImage?)
...
open class PaperOnboarding: UIView {
  ...
  required public init?(coder aDecoder: NSCoder) {
    ...
    NotificationCenter.default.addObserver(self, selector: #selector(PaperOnboarding.refresh), name: NSNotification.Name(rawValue: ConstantsVO.BACKGROUND_CHANGED), object: nil)
  }
    
    public func refresh()
    {
        itemsInfo = createItemsInfo()
        contentView?.currentItem(0, animated: false)
    }
}
