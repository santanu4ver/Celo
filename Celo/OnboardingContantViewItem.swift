...
extension OnboardingContentViewItem {
  ...
  class func itemOnView(_ view: UIView) -> OnboardingContentViewItem {
    ...
    for attribute in [NSLayoutAttribute.top, NSLayoutAttribute.bottom, NSLayoutAttribute.leading, NSLayoutAttribute.trailing] {
        (view, item) >>>- { $0.attribute = attribute; return }
    }
...
private extension OnboardingContentViewItem {
  ...
  func commonInit() {
    ...
    let backgroundImageView = createBackgroundImage(self)
  ...
  func createImage(_ onView: UIView) -> UIImageView {
    ...
    (onView, imageView) >>>- { $0.attribute = .centerX; return }
    topConstraint = (onView, imageView) >>>- { $0.attribute = .top; return }
    
    return imageView
  }
    
    func createBackgroundImage(_ onView: UIView) -> UIImageView {
        let imageView = Init(UIImageView(frame: CGRect.zero)) {
            $0.contentMode                               = .scaleAspectFill
            $0.translatesAutoresizingMaskIntoConstraints = false
        }
        
        onView.insertSubview(imageView, at: 0)
