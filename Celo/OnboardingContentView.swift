...
extension OnboardingContentView {
  ...
  fileprivate func createItem(_ index: Int) -> OnboardingContentViewItem {
    ...
    let item = Init(OnboardingContentViewItem.itemOnView(self)) {
        ...
        // show icon and description only when no background image is set
        if info.backgroundImage != nil
        {
            $0.backgroundImageView?.image = info.backgroundImage
        ...
        $0.titleLabel?.font       = UIFont(name: "HelveticaNeue-Thin", size: 100.0)
...
extension OnboardingContentView {
  ...
  fileprivate func hideItemView(_ item: OnboardingContentViewItem?, duration: Double) {
    ...
    item.topConstraint?.constant -= Constants.dyOffsetAnimation
  ...
  fileprivate func showItemView(_ item: OnboardingContentViewItem, duration: Double) {
    item.bottomConstraint?.constant = Constants.dyOffsetAnimation // animation starts from
    item.centerConstraint?.constant /= 2
    item.topConstraint?.constant = Constants.dyOffsetAnimation
    item.alpha = 0
    layoutIfNeeded()

    item.bottomConstraint?.constant = -110 // animation ends to
    item.centerConstraint?.constant *= 2
    item.topConstraint?.constant = 100
